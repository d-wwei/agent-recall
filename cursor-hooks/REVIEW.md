...
3. **Parity with Claude Code**: Matches Agent Recall's hook behavior where possible
...
**Purpose**: Initialize Agent Recall session when prompt is submitted
...
- ✅ Observation format matches Agent Recall structure
...
**How It Works**:
1. Fetches context from Agent Recall worker
2. Writes to `.cursor/rules/agent-recall-context.mdc` with `alwaysApply: true`
...
6. Write context to `.cursor/rules/agent-recall-context.mdc`
...
3. **Context Injection**:
   - ✅ Solved via auto-updated `.cursor/rules/agent-recall-context.mdc`
