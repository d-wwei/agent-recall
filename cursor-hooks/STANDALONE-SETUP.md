# Agent Recall for Cursor (No Claude Code Required)

> **Persistent AI Memory for Cursor - Zero Cost to Start**

## Overview

Use Agent Recall's persistent memory in Cursor without a Claude Code subscription. Choose between free-tier providers (Gemini, OpenRouter) or paid options.
...
**Why This Matters**: Every Cursor session starts fresh. Agent Recall bridges that gap - your AI agent builds cumulative knowledge about your codebase, decisions, and patterns over time.
...
Since you don't have Claude Code, you need to configure an AI provider for Agent Recall's summarization engine.
...
mkdir -p ~/.agent-recall

# Create settings file
cat > ~/.agent-recall/settings.json << 'EOF'
{
...
```bash
# From the agent-recall repo directory (recommended - all projects)
...
```bash
cat ~/.agent-recall/settings.json
```
...
- Visit https://docs.agent-recall.ai for full documentation
...
$settingsDir = "$env:USERPROFILE\.agent-recall"
