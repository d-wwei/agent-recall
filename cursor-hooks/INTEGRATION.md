# Agent Recall ↔ Cursor Integration Architecture

## Overview

This integration connects Agent Recall's persistent memory system to Cursor's hook system, enabling:
...
session-init.sh
    ...
    └─ Initialize session in Agent Recall
    ↓
context-inject.sh
...
save-observation.sh
    ...
    ├─ Map to Agent Recall observation format
...
| Cursor Field | Agent Recall Field | Notes |
|-------------|------------------|-------|
...
| Cursor Event | Agent Recall Tool Name | Input Format |
|-------------|---------------------|--------------|
...
### Worker Settings
Located in `~/.agent-recall/settings.json`:
...
1. **Context Injection**: Cursor's `beforeSubmitPrompt` doesn't support prompt modification. Context must be retrieved via:
   - MCP tools (Agent Recall provides search tools)
