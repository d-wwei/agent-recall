...
If you want to add custom instructions about Agent Recall (beyond the auto-injected context), create a separate rules file:

### `.cursor/rules/agent-recall-instructions.mdc`

```markdown
---
alwaysApply: true
description: "Instructions for using Agent Recall memory system"
---

# Memory System Usage

You have access to Agent Recall, a persistent memory system. In addition to the auto-injected context above, you can search for more detailed information using MCP tools:
...
|------|---------|------------|
| `.cursor/rules/agent-recall-context.mdc` | Auto-injected context | Hook (automatic) |
| `.cursor/rules/agent-recall-instructions.mdc` | MCP tool instructions | You (optional) |
