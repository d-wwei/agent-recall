# Agent Recall Cursor Hooks Integration
...
```bash
# Clone and build
git clone https://github.com/user/agent-recall.git
cd agent-recall && bun install && bun run build
...
The hooks bridge Cursor's hook system to Agent Recall's worker API, allowing:
...
1. **Install**: `agent-recall cursor install` generates initial context
2. **Stop hook**: Updates context in `.cursor/rules/agent-recall-context.mdc` after each session
...
- **MCP Tools**: Configure Agent Recall's MCP server for `search`, `timeline`, `get_observations` tools
...
|-------------|--------|---------|
| `beforeSubmitPrompt` | `session-init.sh` | Initialize Agent Recall session |
...
- Initializes a new session in Agent Recall using `conversation_id` as the session ID
- Extracts project name from workspace root
- Outputs `{"continue": true}` to allow prompt submission

### Context Hook (`context-inject.sh`)
- Ensures Agent Recall worker is running before session
- Outputs `{"continue": true}` to allow prompt submission
...
- Maps them to Agent Recall's observation format
...
- Requests summary generation from Agent Recall
- **Updates context file** in `.cursor/rules/agent-recall-context.mdc` for next session

## Configuration

The hooks read configuration from `~/.agent-recall/settings.json`:
...
2. Check worker logs:
   ```bash
   tail -f ~/.agent-recall/logs/worker-$(date +%Y-%m-%d).log
   ```

3. Restart worker:
   ```bash
   agent-recall restart
   ```
...
- [Agent Recall Documentation](https://docs.agent-recall.ai)
- [Cursor Hooks Reference](../docs/context/cursor-hooks-reference.md)
- [Agent Recall Architecture](https://docs.agent-recall.ai/architecture/overview)
