...
async function postToDiscord(webhookUrl, version, notes) {
  ...
  const repoUrl = 'https://github.com/agent-recall/agent-recall';

  const payload = {
    embeds: [
      {
        title: `🚀 Agent Recall ${version} released`,
        ...
        fields: [
          ...
          {
            name: '📚 Docs',
            value: '[docs.agent-recall.ai](https://docs.agent-recall.ai)',
            inline: true,
          },
        ],
        footer: {
          text: 'Agent Recall • Persistent memory for Claude Code',
        },
