...
export async function collectDiagnostics(
  ...
  const homeDir = os.homedir();
  const dataDir = path.join(homeDir, ".agent-recall");
  ...
  const isDevMode = (cwd.includes("agent-recall") || cwd.includes("claude-mem")) && !cwd.includes(".claude");
  ...
  const database = {
    path: sanitizePath(path.join(dataDir, "agent-recall.db")),
...
export function formatDiagnostics(diagnostics: SystemDiagnostics): string {
  ...
  output += "## Environment\n\n";
  output += `- **Agent Recall**: ${diagnostics.versions.claudeMem}\n`;
