/**
 * CursorHooksInstaller - Cursor IDE integration for Agent Recall
 *
...
export function configureCursorMcp(target: CursorInstallTarget): number {
  ...
  if (!mcpServerPath) {
    ...
    console.error('   Expected at: ~/.claude/plugins/marketplaces/agent-recall/plugin/scripts/mcp-server.cjs');
    return 1;
  }
  ...
  try {
    ...
    // Add agent-recall MCP server
    config.mcpServers['agent-recall'] = {
      command: 'node',
      args: [mcpServerPath]
    };
...
export async function installCursorHooks(target: CursorInstallTarget): Promise<number> {
  console.log(`\nInstalling Agent Recall Cursor hooks (${target} level)...\n`);
  ...
  if (!workerServicePath) {
    ...
    console.error('   Expected at: ~/.claude/plugins/marketplaces/agent-recall/plugin/scripts/worker-service.cjs');
    return 1;
  }
...
Next steps:
  1. Start agent-recall worker: agent-recall start
  2. Restart Cursor to load the hooks
  3. Check Cursor Settings → Hooks tab to verify

Context Injection:
  Context from past sessions is stored in .cursor/rules/agent-recall-context.mdc
...
async function setupProjectContext(targetDir: string, workspaceRoot: string): Promise<void> {
  ...
  if (!contextGenerated) {
    ...
    const rulesFile = path.join(rulesDir, 'agent-recall-context.mdc');
...
Use Agent Recall's MCP search tools for manual memory queries.
`;
...
export function uninstallCursorHooks(target: CursorInstallTarget): number {
  console.log(`\nUninstalling Agent Recall Cursor hooks (${target} level)...\n`);
  ...
  try {
    ...
    if (target === 'project') {
      const contextFile = path.join(targetDir, 'rules', 'agent-recall-context.mdc');
...
export function checkCursorHooksStatus(): number {
  console.log('\nAgent Recall Cursor Hooks Status\n');
  ...
  for (const loc of locations) {
    ...
    if (existsSync(hooksJson)) {
      ...
      if (loc.name === 'Project') {
        const contextFile = path.join(loc.dir, 'rules', 'agent-recall-context.mdc');
        if (existsSync(contextFile)) {
  ...
  if (!anyInstalled) {
    console.log('No hooks installed. Run: agent-recall cursor install\n');
  }
...
export async function handleCursorCommand(subcommand: string, args: string[]): Promise<number> {
  switch (subcommand) {
    ...
    default: {
      console.log(`
Agent Recall Cursor Integration

Usage: agent-recall cursor <command> [options]
...
Examples:
  ...
  agent-recall cursor install user       # Install globally for user
  agent-recall cursor uninstall          # Remove from current project
  agent-recall cursor status             # Check if hooks are installed

For more info: https://docs.agent-recall.ai/cursor
      `);
      return 0;
    }
  }
