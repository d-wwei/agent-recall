...
export class OpenRouterAgent {
  ...
  private async queryOpenRouterMultiTurn(
    ...
    const response = await fetch(OPENROUTER_API_URL, {
      ...
      headers: {
        ...
        'HTTP-Referer': siteUrl || 'https://github.com/agent-recall/agent-recall',
        'X-Title': appName || 'Agent Recall',
   ...
   * Issue #733: Uses centralized ~/.agent-recall/.env for credentials, not random project .env files
   */
  private getOpenRouterConfig(): { apiKey: string; model: string; siteUrl?: string; appName?: string } {
    const settingsPath = USER_SETTINGS_PATH;
    const settings = SettingsDefaultsManager.loadFromFile(settingsPath);

    // API key: check settings first, then centralized agent-recall .env (NOT process.env)
    ...
    const appName = settings.CLAUDE_MEM_OPENROUTER_APP_NAME || 'Agent Recall';
 ...
 * Issue #733: Uses centralized ~/.agent-recall/.env, not random project .env files
 */
