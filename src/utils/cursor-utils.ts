...
export function writeContextFile(workspacePath: string, context: string): void {
  ...
  const rulesFile = join(rulesDir, 'agent-recall-context.mdc');
...
The following context is from Agent Recall, a persistent memory system that tracks your coding sessions.

${context}

---
*Updated after last session. Use Agent Recall's MCP search tools for more detailed queries.*
`;
...
export function readContextFile(workspacePath: string): string | null {
  const rulesFile = join(workspacePath, '.cursor', 'rules', 'agent-recall-context.mdc');
...
/**
 * Configure agent-recall MCP server in Cursor's mcp.json
...
/**
 * Remove agent-recall MCP server from Cursor's mcp.json
...
export function removeMcpConfig(mcpJsonPath: string): void {
  ...
  try {
    ...
    if (config.mcpServers && config.mcpServers['agent-recall']) {
      delete config.mcpServers['agent-recall'];
      writeFileSync(mcpJsonPath, JSON.stringify(config, null, 2));
    }
  } catch (e) {
