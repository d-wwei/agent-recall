...
/**
 * Simple path configuration for agent-recall
 * Standard paths based on Claude Code conventions
 */

// Legacy data directory for migration detection
...
function resolveDataDir(): string {
  // 1. Environment variable (highest priority) — check new name first, then legacy
  if (process.env.AGENT_RECALL_DATA_DIR) {
    return process.env.AGENT_RECALL_DATA_DIR;
  }
  if (process.env.CLAUDE_MEM_DATA_DIR) {
    return process.env.CLAUDE_MEM_DATA_DIR;
  }

  // 2. Settings file at the new default location
  const defaultDataDir = join(homedir(), '.agent-recall');
  ...
  try {
    if (existsSync(settingsPath)) {
      ...
      if (settings.AGENT_RECALL_DATA_DIR || settings.CLAUDE_MEM_DATA_DIR) {
        return settings.AGENT_RECALL_DATA_DIR || settings.CLAUDE_MEM_DATA_DIR;
      }
    }
  } catch {
  ...
  // 3. Check legacy location for migration
  const legacySettingsPath = join(LEGACY_DATA_DIR, 'settings.json');
  try {
    if (existsSync(legacySettingsPath)) {
      const { readFileSync } = require('fs');
      const raw = JSON.parse(readFileSync(legacySettingsPath, 'utf-8'));
    ...
    // legacy settings missing or corrupt — fall through to default
  }
...
export const MARKETPLACE_ROOT = join(CLAUDE_CONFIG_DIR, 'plugins', 'marketplaces', 'agent-recall');
// Legacy marketplace path for migration
export const LEGACY_MARKETPLACE_ROOT = join(CLAUDE_CONFIG_DIR, 'plugins', 'marketplaces', 'thedotmack');
...
export const DB_PATH = join(DATA_DIR, 'agent-recall.db');
export const LEGACY_DB_NAME = 'claude-mem.db';
