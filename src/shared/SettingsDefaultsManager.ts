 ...
 *
 * Backward compatibility: All CLAUDE_MEM_* env vars are still accepted.
 * New AGENT_RECALL_* env vars take priority when both are set.
 */
...
export interface SettingsDefaults {
  ...
  CLAUDE_MEM_RATE_LIMIT_PAUSE_SECONDS: string;  // Pause duration after repeated SDK agent rate limits (default: 300)
  // Exclusion Settings
  CLAUDE_MEM_EXCLUDED_PROJECTS: string;  // Comma-separated glob patterns for excluded project paths
  CLAUDE_MEM_FOLDER_MD_EXCLUDE: string;  // JSON array of folder paths to exclude from CLAUDE.md generation
  // Vector Search Backend
  AGENT_RECALL_VECTOR_BACKEND: string; // 'seekdb' | 'none' - default 'seekdb'
...
export class SettingsDefaultsManager {
  ...
  private static readonly DEFAULTS: SettingsDefaults = {
    ...
    CLAUDE_MEM_OPENROUTER_APP_NAME: 'agent-recall',  // App name for OpenRouter analytics
    CLAUDE_MEM_OPENROUTER_MAX_CONTEXT_MESSAGES: '20',  // Max messages in context window
    CLAUDE_MEM_OPENROUTER_MAX_TOKENS: '100000',  // Max estimated tokens (~100k safety limit)
    // System Configuration
    CLAUDE_MEM_DATA_DIR: join(homedir(), '.agent-recall'),
    ...
    CLAUDE_MEM_RATE_LIMIT_PAUSE_SECONDS: '300',  // Pause for 5 minutes after 3 consecutive rate limits
    // Exclusion Settings
    CLAUDE_MEM_EXCLUDED_PROJECTS: '',  // Comma-separated glob patterns for excluded project paths
    CLAUDE_MEM_FOLDER_MD_EXCLUDE: '[]',  // JSON array of folder paths to exclude from CLAUDE.md generation
    // Vector Search Backend
    AGENT_RECALL_VECTOR_BACKEND: 'seekdb',     // 'seekdb' (embedded, default) or 'none' (SQLite-only)
    // Data Retention / Lifecycle Cleanup
