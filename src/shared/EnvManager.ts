/**
 * EnvManager - Centralized environment variable management for agent-recall
 *
 * Provides isolated credential storage in ~/.agent-recall/.env
 * This ensures agent-recall uses its own configured credentials,
 * not random ANTHROPIC_API_KEY values from project .env files.
 *
 * Issue #733 (upstream): SDK was auto-discovering API keys from user's shell environment,
...
// Path to agent-recall's centralized .env file (with legacy fallback)
const DATA_DIR = join(homedir(), '.agent-recall');
const LEGACY_DATA_DIR = join(homedir(), '.claude-mem');
export const ENV_FILE_PATH = join(DATA_DIR, '.env');
const LEGACY_ENV_FILE_PATH = join(LEGACY_DATA_DIR, '.env');
...
export interface AgentRecallEnv {
...
/** @deprecated Use AgentRecallEnv instead */
export type ClaudeMemEnv = AgentRecallEnv;
...
function serializeEnvFile(env: Record<string, string>): string {
  const lines: string[] = [
    '# agent-recall credentials',
    '# This file stores API keys for agent-recall memory agent',
    '# Edit this file or use agent-recall settings to configure',
    '',
  ];
...
/**
 * Load credentials from ~/.agent-recall/.env (or legacy ~/.claude-mem/.env)
 * Returns empty object if file doesn't exist (means use CLI billing)
 */
export function loadClaudeMemEnv(): AgentRecallEnv {
  // Check new path first, then legacy
  const envPath = existsSync(ENV_FILE_PATH) ? ENV_FILE_PATH : LEGACY_ENV_FILE_PATH;
...
/**
 * Save credentials to ~/.agent-recall/.env
 */
export function saveClaudeMemEnv(env: AgentRecallEnv): void {
  try {
 ...
 *
 * If agent-recall has an explicit ANTHROPIC_API_KEY in ~/.agent-recall/.env, it's re-injected
 ...
 */
export function getCredential(key: keyof AgentRecallEnv): string | undefined {
 ...
 */
export function setCredential(key: keyof AgentRecallEnv, value: string): void {
  const env = loadClaudeMemEnv();
...
export function getAuthMethodDescription(): string {
  if (hasAnthropicApiKey()) {
    return 'API key (from ~/.agent-recall/.env)';
  }
