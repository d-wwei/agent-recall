...
const TEST_DIR = join(tmpdir(), `agent-recall-smart-install-test-${process.pid}`);
...
describe('smart-install resolveRoot logic', () => {
  ...
  it('should prefer CLAUDE_PLUGIN_ROOT when it contains package.json', () => {
    const cacheDir = createDir('cache/agent-recall/agent-recall/10.0.0');
  ...
  it('should detect cache-based install paths', () => {
    // Cache installs have paths like ~/.claude/plugins/cache/agent-recall/agent-recall/<version>/
    const cacheDir = createDir('plugins/cache/agent-recall/agent-recall/10.3.0');
    createPackageJson(cacheDir);
...
describe('smart-install stdout JSON output (#1253)', () => {
  ...
  it('should produce valid JSON when run with plugin disabled', () => {
    ...
    const settingsDir = join(tmpdir(), `agent-recall-test-settings-${process.pid}`);
