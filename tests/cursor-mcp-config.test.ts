...
describe('Cursor MCP Configuration', () => {
  ...
  describe('configureCursorMcp', () => {
    ...
    it('adds agent-recall server with correct structure', () => {
      configureCursorMcp(mcpJsonPath, mcpServerPath);

      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));

      expect(config.mcpServers).toBeDefined();
      expect(config.mcpServers['agent-recall']).toBeDefined();
      expect(config.mcpServers['agent-recall'].command).toBe('node');
      expect(config.mcpServers['agent-recall'].args).toEqual([mcpServerPath]);
    });

    it('preserves existing MCP servers when adding agent-recall', () => {
    ...
    it('updates existing agent-recall server path', () => {
      // First config
      ...
      expect(config.mcpServers['agent-recall'].args).toEqual([newPath]);
    });
  ...
  describe('removeMcpConfig', () => {
    it('removes agent-recall server from config', () => {
      configureCursorMcp(mcpJsonPath, mcpServerPath);
      removeMcpConfig(mcpJsonPath);

      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['agent-recall']).toBeUndefined();
    });

    it('preserves other servers when removing agent-recall', () => {
      // Setup: both servers
      mkdirSync(join(tempDir, '.cursor'), { recursive: true });
      const config = {
        mcpServers: {
          'other-server': { command: 'python', args: ['/path.py'] },
          'agent-recall': { command: 'node', args: ['/mcp.cjs'] }
        }
      };
      ...
      expect(updated.mcpServers['agent-recall']).toBeUndefined();
    });
    ...
    it('does nothing if agent-recall not in config', () => {
      mkdirSync(join(tempDir, '.cursor'), { recursive: true });
      const config = {
        mcpServers: {
          'other-server': { command: 'python', args: ['/path.py'] }
  ...
  describe('path handling', () => {
    it('handles absolute path with spaces', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['agent-recall'].args).toEqual([pathWithSpaces]);
    });
    ...
    it('handles Windows-style path', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['agent-recall'].args).toEqual([windowsPath]);
    });
    ...
    it('handles path with special characters', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['agent-recall'].args).toEqual([specialPath]);

      // Verify it survives JSON round-trip
      const reread: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(reread.mcpServers['agent-recall'].args![0]).toBe(specialPath);
    });
  });
});
