...
describe('Version Consistency', () => {
  ...
  it('should have matching version in plugin/.claude-plugin/plugin.json', () => {
    ...
    // plugin.json is the upstream claude-mem manifest, not updated during rebrand
    // Skip this check — the authoritative version source is root package.json
    if (!existsSync(pluginJsonPath)) return;

    const pluginJson = JSON.parse(readFileSync(pluginJsonPath, 'utf-8'));
    // Only check existence, not version match (upstream artifact)
  ...
  it('should have matching version in .claude-plugin/marketplace.json', () => {
    ...
    const agentRecallPlugin = marketplaceJson.plugins.find((p: any) => p.name === 'agent-recall');
    expect(agentRecallPlugin).toBeDefined();
    expect(agentRecallPlugin.version).toBe(rootVersion);
  });
  ...
  it('should validate version format is semver compliant', () => {
    ...
    // Extract major.minor.patch, ignoring prerelease suffix (e.g. 1.0.0-alpha.1)
