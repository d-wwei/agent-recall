...
`)}getCompletenessScore(e){let t=this.db.prepare(`
    ...
    `),this.db.run("CREATE INDEX IF NOT EXISTS idx_doctor_reports_created ON doctor_reports(created_at DESC)"),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(46,new Date().toISOString()),l.debug("DB","Created doctor_reports table"))}addDoctorDeepAnalysisColumn(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(47))return;this.db.prepare("PRAGMA table_info(doctor_reports)").all().some(n=>n.name==="deep_analysis")||(this.db.run("ALTER TABLE doctor_reports ADD COLUMN deep_analysis TEXT"),l.debug("DB","Added deep_analysis column to doctor_reports")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(47,new Date().toISOString())}};var le=class{db;constructor(e=nt){e!==":memory:"&&rt(x),this.db=new at.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),new ce(this.db).runAllMigrations()}initializeSchema(){this.db.run(`
      CREATE TABLE IF NOT EXISTS schema_versions (
        id INTEGER PRIMARY KEY,
        version INTEGER UNIQUE NOT NULL,
        applied_at TEXT NOT NULL
